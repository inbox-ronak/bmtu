...
class Magazine_subscription extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->dbTable = 'magazine_subscription_master';
	}
	public function data()
	{
		...
		$permission = $this->permission->grant(MAGAZINE_SUBSCRIPTION,'view');
		if($permission == false){
			redirect('admin/dashboard');
		}
		$edit_permission = $this->permission->grant(MAGAZINE_SUBSCRIPTION,'edit');
		$delete_permission = $this->permission->grant(MAGAZINE_SUBSCRIPTION,'delete');
		$data = array();
            ...
            if(isset($data) && !empty($data)) {
               ...
               ?>
                  ...
                  <td>
                  	...
                  	<a href="<?php echo base_url();?>admin/magazine_subscription/edit/<?php echo base64_encode($row['id']);?>" class="btn btn-info btn-sm item_edit"><i class="fa fa-edit"></i></a>
                  	<?php } ?>
                  	<?php //if($row['id'] > 3){ ?>
	...
	public function Index(){
		...
		$data = array();
		$data['main_page'] = 'backend/magazine_subscription/list';
	...
	public function add()
	{
		...
		$permission = $this->permission->grant(MAGAZINE_SUBSCRIPTION,'add');
		if($permission == false)
		{
			redirect('admin/dashboard');
		}
	    ...
	    $smoi_chapter = trim($this->input->post('smoi_chapter'));
        ...
        $response = array();
        if($insert){
        	$this->session->set_flashdata('success','Magazine Subscription is created successfully.');
        }else{
        	$this->session->set_flashdata('error','Something went wrong.');
        }
        redirect('admin/magazine_subscription');
      }else{
        $data = array();
      	$data['main_page'] = 'backend/magazine_subscription/add';
				$this->load->view('layout/template',$data);
      }
	}
	public function edit($id)
	{
		...
		$permission = $this->permission->grant(MAGAZINE_SUBSCRIPTION,'edit');
		if($permission == false)
		{
			redirect('admin/dashboard');
		}
        ...
        //print_r($update);die;
        if($update){
        	 $this->session->set_flashdata('success','Magazine is updated successfully.');
      	}else{
        	$this->session->set_flashdata('error','Something went wrong.');
      	}
      redirect('admin/magazine_subscription');
    	...
    	$where_array = array('id' => $id);
    	$data['renewal'] = $this->common_model->get_records('magazine_subscription_master','',$where_array,true);
  ...
  {
    $permission = $this->permission->grant(MAGAZINE_SUBSCRIPTION,'delete');
    ...
    if($delete){
      $response['success'] = 1;
      $response['message'] = 'magazine subscription is deleted successfully.';
    }else{
      $response['success'] = 0;
