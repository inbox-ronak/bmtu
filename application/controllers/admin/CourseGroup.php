...
class CourseGroup extends CI_Controller {
	...
	public function data()
	{
		...
		$permission = $this->permission->grant(COURSE_GROUP,'view');
		if($permission == false){
			redirect('admin/dashboard');
		}
		$edit_permission = $this->permission->grant(COURSE_GROUP,'edit');
		$delete_permission = $this->permission->grant(COURSE_GROUP,'delete');
		$data = array();
            ...
            if(isset($data) && !empty($data)) {
               ...
               ?>
                  ...
                  <td>
                  	...
                  	<a href="<?php echo base_url();?>admin/CourseGroup/edit/<?php echo base64_encode($row['id']);?>" class="btn btn-info btn-sm item_edit"><i class="fa fa-edit"></i></a>
                  	<?php } ?>
                  	<?php //if($row['id'] > 3){ ?>
	...
	public function Index(){
		...
		$data = array();
		$data['main_page'] = 'backend/course_group/list';
	...
	public function add()
	{
		...
		$permission = $this->permission->grant(COURSE_GROUP,'add');
		if($permission == false)
		{
			redirect('admin/dashboard');
		}
        ...
        $response = array();
        if($insert){
        	$this->session->set_flashdata('success','CourseGroup created successfully.');
        }else{
        	$this->session->set_flashdata('error','Something went wrong.');
        }
        redirect('admin/CourseGroup');
      }else{
        $where_arr = array('status=' => 1);
        $data['colleges'] = $this->common_model->get_records($this->dbTable2,'',$where_arr);
        // $data = array();
      	$data['main_page'] = 'backend/course_group/add';
		$this->load->view('layout/template',$data);
      }
	}
	public function edit($id)
	{
		...
		$permission = $this->permission->grant(COURSE_GROUP,'edit');
		if($permission == false)
		{
			redirect('admin/dashboard');
		}
  ...
  {
    $permission = $this->permission->grant(COURSE_GROUP,'delete');
    ...
    if($delete){
      $response['success'] = 1;
      $response['message'] = 'CourseGroup is deleted successfully.';
    }else{
      $response['success'] = 0;
