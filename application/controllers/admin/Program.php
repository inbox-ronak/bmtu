...
class Program extends CI_Controller {
	...
	public function data()
	{
		...
		$permission = $this->permission->grant(PROGRAM,'view');
		if($permission == false){
			redirect('admin/dashboard');
		}
		$edit_permission = $this->permission->grant(PROGRAM,'edit');
		$delete_permission = $this->permission->grant(PROGRAM,'delete');
		$data = array();
            ...
            if(isset($data) && !empty($data)) {
               ...
               ?>
                  ...
                  <td>
                  	...
                  	<a href="<?php echo base_url();?>admin/Program/edit/<?php echo base64_encode($row['id']);?>" class="btn btn-info btn-sm item_edit"><i class="fa fa-edit"></i></a>
                  	<?php } ?>
                  	<?php //if($row['id'] > 3){ ?>
	...
	public function Index(){
		...
		$data = array();
		$data['main_page'] = 'backend/program/list';
	...
	public function add()
	{
		...
		$permission = $this->permission->grant(PROGRAM,'add');
		if($permission == false)
		{
			redirect('admin/dashboard');
		}
        ...
        $response = array();
        if($insert){
        	$this->session->set_flashdata('success','Program created successfully.');
        }else{
        ...
        // $data = array();
      	$data['main_page'] = 'backend/program/add';
		$this->load->view('layout/template',$data);
      }
	}
	public function edit($id)
	{
		...
		$permission = $this->permission->grant(PROGRAM,'edit');
		if($permission == false)
		{
			redirect('admin/dashboard');
		}
        ...
        // print_r($update);die;
        if($update){
        	 $this->session->set_flashdata('success','Program is updated successfully.');
      	}else{
        	$this->session->set_flashdata('error','Something went wrong.');
      	}
      redirect('admin/Program');
  ...
  {
    $permission = $this->permission->grant(PROGRAM,'delete');
    ...
    if($delete){
      $response['success'] = 1;
      $response['message'] = 'Program is deleted successfully.';
    }else{
      $response['success'] = 0;
