...
class CourseComponent extends CI_Controller {
	...
	public function data()
	{
		...
		$permission = $this->permission->grant(COURSE_COMPONENT,'view');
		if($permission == false){
			redirect('admin/dashboard');
		}
		$edit_permission = $this->permission->grant(COURSE_COMPONENT,'edit');
		$delete_permission = $this->permission->grant(COURSE_COMPONENT,'delete');
		$data = array();
		...
		<table id="example2" class="table table-bordered table-striped">
            ...
            if(isset($data) && !empty($data)) {
               ...
               ?>
                  ...
                  <td><?php echo $row['course_component_name']; ?></td>
                  <td><?php echo $status; ?></td>
                  <td>
                  	<?php if($edit_permission == true){ ?>
                  	<a href="<?php echo base_url();?>admin/CourseComponent/edit/<?php echo base64_encode($row['id']);?>" class="btn btn-info btn-sm item_edit"><i class="fa fa-edit"></i></a>
                  	<?php } ?>
                  	<?php //if($row['id'] > 3){ ?>
	...
	public function Index(){
		...
		$data = array();
		$data['main_page'] = 'backend/course_component/list';
	...
	public function add()
	{
		...
		$permission = $this->permission->grant(COURSE_COMPONENT,'add');
		if($permission == false)
		{
			redirect('admin/dashboard');
		}
		if(isset($_POST['submit']))
		{
			$data = array(
						...
						'course_component_name'=>$this->input->post('course_component_name'),
	...
	public function edit($id)
	{
		...
		$permission = $this->permission->grant(COURSE_COMPONENT,'edit');
		if($permission == false)
		{
			redirect('admin/dashboard');
		}
        ...
        // print_r($update);die;
        if($update){
        	 $this->session->set_flashdata('success','Course Group is updated successfully.');
      	}else{
        	$this->session->set_flashdata('error','Something went wrong.');
      	}
      redirect('admin/CourseComponent');
  ...
  {
    $permission = $this->permission->grant(COURSE_COMPONENT,'delete');
