...
class StudentDetails extends CI_Controller {
	...
	public function data()
	{
		...
		$permission = $this->permission->grant(STUDENT_DETAILS,'view');
		if($permission == false){
			redirect('admin/dashboard');
		}
		$edit_permission = $this->permission->grant(STUDENT_DETAILS,'edit');
		$delete_permission = $this->permission->grant(STUDENT_DETAILS,'delete');
		$data = array();
	...
	?>
		<table id="example2" class="table table-bordered table-striped">
                ...
                <tr>
                  <th>#</th>
                  <th>Student Id</th>
                  <th>PWP Certificate</th>
                  <th>Physically Hendicap</th>
                  <th>Date of Birth(DOB) as per SSC</th>
                  <th>DOB as per SSC and Aadhar are same?</th>
                  <th>ST/SC/EWC/OBC Certificate No.</th>
                  <th>Certificate Validity Date</th>
                  <th>Non-Creamy Layer Date</th>
                  <th>Action</th>
                </tr>
                </thead>
                <tbody>
            ...
            if(isset($data) && !empty($data)) {
               ...
               ?>
                  ...
                  <td><?php echo $row['pwp_certificate']; ?></td>
                  <td><?php echo $row['physically_hendicap']; ?></td>
                  <td><?php echo $row['dob_ssc']; ?></td>
                  <td><?php echo $row['dob_ssc_aadhar']; ?></td>
                  <td><?php echo $row['st_sc_ewc_obc_certificate']; ?></td>
                  <td><?php echo $row['certificate_validity_date']; ?></td>
                  <td><?php echo $row['non_creamy_date']; ?></td>
                  <td>
                  	<?php if($edit_permission == true){ ?>
                  	<!-- <a href="<?php echo base_url();?>admin/StudentDetails/edit/<?php echo base64_encode($row['id']);?>" class="btn btn-info btn-sm item_edit"><i class="fa fa-edit"></i></a> -->
                  	<?php } ?>
                  	<?php //if($row['id'] > 3){ ?>
	...
	public function Index(){
		...
		$data = array();
		$data['main_page'] = 'backend/student_details/list';
	...
	public function add()
	{
		...
		$permission = $this->permission->grant(STUDENT_DETAILS,'add');
		if($permission == false)
		{
			redirect('admin/dashboard');
		}
        ...
        $data['student_ids'] = $this->common_model->get_records($this->dbTable2,'',$where_arr);
      	$data['main_page'] = 'backend/student_details/add';
				$this->load->view('layout/template',$data);
      }
	}
	public function edit($id)
	{
		...
		$permission = $this->permission->grant(STUDENT_DETAILS,'edit');
		if($permission == false)
		{
			redirect('admin/dashboard');
		}
  ...
  {
    $permission = $this->permission->grant(STUDENT_DETAILS,'delete');
