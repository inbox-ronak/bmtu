...
class Department extends CI_Controller {
	...
	public function data()
	{
		...
		$permission = $this->permission->grant(DEPARTMENT,'view');
		if($permission == false){
			redirect('admin/dashboard');
		}
		$edit_permission = $this->permission->grant(DEPARTMENT,'edit');
		$delete_permission = $this->permission->grant(DEPARTMENT,'delete');
		$data = array();
            ...
            if(isset($data) && !empty($data)) {
               ...
               ?>
                  ...
                  <td>
                  	...
                  	<a href="<?php echo base_url();?>admin/Department/edit/<?php echo base64_encode($row['id']);?>" class="btn btn-info btn-sm item_edit"><i class="fa fa-edit"></i></a>
                  	<?php } ?>
                  	<?php //if($row['id'] > 3){ ?>
	...
	public function Index(){
		...
		$data = array();
		$data['main_page'] = 'backend/department/list';
	...
	public function add()
	{
		...
		$permission = $this->permission->grant(DEPARTMENT,'add');
		if($permission == false)
		{
			redirect('admin/dashboard');
		}
        ...
        // $data = array();
      	$data['main_page'] = 'backend/department/add';
		$this->load->view('layout/template',$data);
      }
	}
	public function edit($id)
	{
		...
		$permission = $this->permission->grant(DEPARTMENT,'edit');
		if($permission == false)
		{
			redirect('admin/dashboard');
		}
      ...
      redirect('admin/Department');
        ...
        $data['colleges'] = $this->common_model->get_records($this->dbTable2,'',$where_arr);
    	$where_array = array('id' => $id);
    	$data['record'] = $this->common_model->get_records('department','',$where_array,true);
  ...
  {
    $permission = $this->permission->grant(DEPARTMENT,'delete');
